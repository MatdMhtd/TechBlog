class MyStrings {
  static const String viewHotestBlog = "مشاهده داغ ترین نوشته‌ها";
  static const String viewHotestPodcast = "مشاهده داغ ترین پادکست‌ها";
  static const String relatedArticale = "نوشته‌های مرتبط";
  static const String submitArticale = "ارسال مقاله";
  static const String submitPodcast = "ارسال پادکست";
  static const String editedProfile = "ویرایش عکس حساب کاربری";
  static const String myFavArticle = "مقالات مورد علاقه";
  static const String myFavPodcast = "پادکست‌های مورد علاقه";
  static const String signOut = "خروج از حساب کاربری";
  static const String welcomSignUp = """به تک‌بلاگ خوش آمدید
  
  برای ارسال مطلب و پادکست باید حتما
   ثبت نام کنید""";

  static const String btnSignUp = "بزن بریم";
}
